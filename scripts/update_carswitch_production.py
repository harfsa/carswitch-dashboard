...
def unavailable(text):
    ...
    return any(x in t for x in [
        ...
        "listing is no longer available","no longer available","this listing has been removed",
        "listing removed","غير متوفر","غير متاحة","تم حذف الإعلان","الإعلان غير متوفر"
    ])
...
async def check(page,car):
    ...
    try:
        ...
        if status in (404,410): return car,"gone",f"HTTP {status}"
...
async def main():
    ...
    payload=json.loads(DATA.read_text(encoding="utf-8")); all_cars=payload.get("cars",[])
    if not isinstance(all_cars,list) or not all_cars: raise SystemExit("Dataset contains no cars; refusing to modify it.")
    cars=all_cars
    if TEST_URLS:
        by_url={c.get("url") or c.get("listing_url") or c.get("link"): c for c in all_cars}
        missing=[u for u in TEST_URLS if u not in by_url]
        if missing: raise SystemExit("TEST_URLS not found in dataset: " + ", ".join(missing))
    ...
    report={"generated_at":now,"checked":len(results),"dataset_total":len(all_cars),"counts":counts,"deleted":0,"dry_run":DRY_RUN,"test_urls":TEST_URLS,"policy":"Only verified active records would be updated; no records deleted.","changes":[{"url":old.get("url"),"state":state,"reason":reason,"old_price":old.get("price"),"new_price":new.get("price"),"old_mileage":old.get("mileage"),"new_mileage":new.get("mileage"),"old_year":old.get("year"),"new_year":new.get("year")} for old,new,state,reason in results if state=="active"]}
    ...
    updated_by_url={new.get("url"):new for old,new,state,_ in results if state=="active"}
    updated=[updated_by_url.get(c.get("url") or c.get("listing_url") or c.get("link"),c) for c in all_cars]
