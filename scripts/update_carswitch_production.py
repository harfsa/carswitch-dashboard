...
async def main():
    ...
    async with async_playwright() as p:
        ...
        for i, car in enumerate(cars, 1):
            ...
            transitioned, action, streak_before, streak_after = apply_availability_state(carried, state)
    ...
    report = {
        "generated_at": now, "checked": len(results), "dataset_total": len(all_cars),
        "counts": counts, "deleted": 0, "would_delete": would_delete,
        "dry_run": DRY_RUN, "test_urls": TEST_URLS,
        "policy": "A record is deleted only after two consecutive definitive 'gone' checks. A temporary or unparsed check never causes deletion and preserves the current gone streak for the next definitive check.",
        "changes": [
            {"url": old.get("url"), "state": state, "reason": reason, "action": action,
             "gone_streak_before": streak_before, "gone_streak_after": streak_after,
